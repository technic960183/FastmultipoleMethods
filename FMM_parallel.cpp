...
int main()
{
    ...
    for (id = 0; id < N; id++)
    {
        //if (omp_get_thread_num()==0){std::cout<<id<<std::endl;}
        for (int lv = 2; lv < Level; lv++)
        {
            int *center_idx = &particles_idx[Tran_idx_pt(id, 0, 0)];
            //int *incl = NeighboursChildRange(&particles_idx[Tran_idx_pt(id, 0, 0)], lv - 1);
            int center_level_incl = lv-1;
            int incl[3 * 2];
            for (int k = 0; k < 3; k++)
                {
                    int a = 2 * (center_idx[3 * center_level_incl + k] - 1);
                    int b = 2 * (center_idx[3 * center_level_incl + k] + 1) + 1;
                    incl[k] = (0 > a) ? 0 : a;
                    incl[k + 3] = (Pow2[center_level_incl + 1] - 1 < b) ? Pow2[center_level_incl + 1] - 1 : b;
                } 
            //       
            //int *excl = NeighboursRange(&particles_idx[Tran_idx_pt(id, 0, 0)], lv);
            int center_level_excl = lv;
            int excl[3 * 2];
            for (int k = 0; k < 3; k++)
            {
                excl[k] = (0 > center_idx[3 * center_level_excl + k] - 1) ? 0 : center_idx[3 * center_level_excl + k] - 1;
                excl[k + 3] = (Pow2[center_level_excl] - 1 < center_idx[3 * center_level_excl + k] + 1) ? Pow2[center_level_excl] - 1 : center_idx[3 * center_level_excl + k] + 1;
            }
            //
            for (int x = incl[0]; x <= incl[3]; x++)
                for (int y = incl[1]; y <= incl[4]; y++)
                    for (int z = incl[2]; z <= incl[5]; z++)
                    {
                        ...
                        //double *center = CellCenter(LToXYZ(L, lv), lv);
                        double center[3];
                        for (int i=0;i<3;i++){center[i] = LToXYZ(L,lv)[i]* pow(2, -lv) + pow(2, -lv - 1);}
                        //
                        // ***********************************************
                        // ***********************************************
                        ...
                        //double *corr_sph = CarToSph(deltaX);
                        double corr_sph[3];
                        double r = sqrt(pow(deltaX[0], 2) + pow(deltaX[1], 2) + pow(deltaX[2], 2));
                        double theta = acos(deltaX[2] / r);
                        double phi = atan2(deltaX[1], deltaX[0]);
                        corr_sph[0] = r;
                        corr_sph[1] = theta;
                        corr_sph[2] = phi;
                        //
    ...
    //std::cout << (b-a)/1000000./num_thr << std::endl;
